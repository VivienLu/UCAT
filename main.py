...
def parse_option():
    ...
    parser.add_argument('--model_dir', type=str, default='../results/UCAT/models', 
    ...
    parser.add_argument('--save_dir', type=str, default='../results/UCAT/', 
                        help='path to save models')
    parser.add_argument('--tau', type=float, default=1e5, help='gpu to use')
...
def train(train_loader, texts, model, frozen_model, prompter, add_prompter,
    ...
    for i, (images, target) in enumerate(tqdm(train_loader)):
        ...
        writer.add_scalar('Loss/UCAT', kl_adv2ori, iter_num)
        ...
        for i, (images, target) in enumerate(tqdm(val_loader)):
            ...
            with autocast():
                ...
                with torch.no_grad():
                    ...
                    class_num = output_org.shape[1]
                    alpha_clean = 1e-6 + torch.exp(nn.Softplus()(output_org))
                    au_clean = au_dirichlet(alpha_clean, class_num).mean()
                    eu_clean = eu_dirichlet(alpha_clean, class_num).mean()
                    pu_clean = entropy_from_logits(output_org).mean()
                    ...
                    alpha_ori = 1e-6 + torch.exp(nn.Softplus()(output_ori))
                    au_ori = au_dirichlet(alpha_ori, class_num).mean()
                    eu_ori = eu_dirichlet(alpha_ori, class_num).mean()
                    pu_ori = entropy_from_logits(output_ori).mean()
                ...
                if args.attack == 'pgd':
                    ...
                    delta_prompt = attack_CW(prompter, model, add_prompter, images, target, text_tokens, test_stepsize,
                                                    args.test_numsteps, 'l_inf', device, epsilon=args.test_eps)
                ...
                with torch.no_grad():
                    output_org_adv, _, _= multiGPU_CLIP(model, clip_img_preprocessing(attacked_images,device),
                                                        text_tokens, target, device, None)
                    output_org_adv_oriclip, _, _ = multiGPU_CLIP(frozen_model, clip_img_preprocessing(attacked_images_oriclip,device),
                                                        text_tokens, target, device, None)
                    
                    alpha_adv = 1e-6 + torch.exp(nn.Softplus()(output_org_adv))
                    au_adv = au_dirichlet(alpha_adv, class_num).mean()
                    eu_adv = eu_dirichlet(alpha_adv, class_num).mean()
                    pu_adv = entropy_from_logits(output_org_adv).mean()
            ...
            logger.info(f'\nValidating:\n= Original. acc. {top1_ori.avg:.4f} au. {au_ori.item():.4f} eu. {eu_ori.item():.4f} pu. {pu_ori.item():.4f}')
            logger.info(f'= clean. acc. {top1_org.avg:.4f} au. {au_clean.item():.4f} eu. {eu_clean.item():.4f} pu. {pu_clean.item():.4f}')
            logger.info(f'= Adv. acc. {acc1[0].item():.4f} au. {au_adv.item():.4f} eu. {eu_adv.item():.4f} pu. {pu_adv.item():.4f}')
