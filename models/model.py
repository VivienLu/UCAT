...
def multiGPU_CLIP_image_logits(images, model, text_tokens,target, device, prompter=None, add_prompter=None, eva_clip=False, slip=False):
    ...
    return multiGPU_CLIP(model, image_tokens, text_tokens, target, device, prompt_token=prompt_token, eva_clip=eva_clip, slip=slip)[0]

def multiGPU_CLIP(clip_model, images, text_tokens, target, device, prompt_token=None, eva_clip=False, slip=False):
    ...
    if text_tokens.size()[0] == 1000:
        ...
        if eva_clip:
            ...
            text_embed = clip_model.encode_text(text_tokens)#torch.Size([100, 77])
            text_embed = text_embed / text_embed.norm(dim=-1, keepdim=True)
            scale_text_embed = 100.0 * text_embed
        elif slip:
            text_features = slip_utils.get_model(clip_model).encode_text(text_tokens)
            text_embed = text_features / text_features.norm(dim=-1, keepdim=True)
            img_embed = slip_utils.get_model(clip_model).encode_image(images)
            img_embed = img_embed / img_embed.norm(dim=-1, keepdim=True)
            scale_text_embed = 100.0 * text_embed
        else:
...
def multiGPU_CLIP_image_logits_caa(images, model, text_tokens,target, device, prompter=None, add_prompter=None):
    image_tokens = clip_img_preprocessing(images, device)
    prompt_token = None if add_prompter is None else add_prompter()
    if prompter is not None:
        image_tokens = prompter(image_tokens)
    return multiGPU_CLIP_caa(model, image_tokens, text_tokens, target, device, prompt_token=prompt_token)[0]


def multiGPU_CLIP_caa(clip_model, images, text_tokens, target, device, prompt_token=None):
    if prompt_token is not None:
        bs = images.size(0)
        prompt_token = prompt_token.repeat(bs, 1, 1)

    if text_tokens.size(0) == 1000:
        img_embed = clip_model.visual(images, prompt_token)[:, 0, :]   # 或 clip_model.image_encoder(...)
        img_embed = img_embed / img_embed.norm(dim=-1, keepdim=True)
